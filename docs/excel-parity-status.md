...
| Formula evaluator (`FormulaEvaluator`) | Partial | Display-time evaluation for a growing high-frequency function set and cross-sheet references; current support includes `SUM`, `AVERAGE`, `MIN`, `MAX`, `COUNT`, `IF`, `COUNTA`, `SUMPRODUCT`, `INDEX`, `MATCH`, `VLOOKUP`, `HLOOKUP`, `SUMIF`, `COUNTIF`, `AVERAGEIF`, `SUMIFS`, `COUNTIFS`, `AVERAGEIFS`, `MAXIFS`, `MINIFS`, selected text helpers (`LEN`, `TRIM`, `UPPER`, `LEFT`, `RIGHT`, `MID`, `CONCATENATE`, `TEXTJOIN`), and selected date/time helpers (`DATE`, `YEAR`, `MONTH`, `DAY`, `DAYS`, `DATEDIF`, `NETWORKDAYS`, `WORKDAY`, `EOMONTH`) | Large remaining function matrix still includes many OfficeCLI areas: more text/date/time/logical/statistical functions, more lookup edge cases, more nested-expression support, richer Excel-style coercion/error propagation, error-array semantics, and more robust circular/lookup behavior | Very high |
